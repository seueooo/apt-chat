...
pnpm workspace 기반 모노레포. 백엔드(`apps/server/`), 프런트엔드(`apps/web/`), 데이터 수집(`apps/etl/`), 배포(`deploy/`), 문서(`docs/`)가 단일 저장소에 공존한다.
...
├── package.json                    # workspace 루트 (packageManager: pnpm@9.4.0)
├── pnpm-workspace.yaml             # apps/* 워크스페이스 정의
│
├── apps/                           # 실행 단위 (server / web / etl)
...
│       └── transform.py            # 정규화
│
├── deploy/                         # Docker 배포 산출물
│   ├── Dockerfile.server           # python:3.12-slim, uvicorn
│   ├── Dockerfile.web              # node:20-alpine, multi-stage standalone
│   └── docker-compose.yml          # server + web (depends_on)
│
└── docs/                           # 이 문서들
    ├── README.md
    ├── architecture/
